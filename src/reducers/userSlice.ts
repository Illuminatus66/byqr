...
const userSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(signup.fulfilled, (state, action: PayloadAction<{ token: string; result: User }>) => {
        state.data = { token: action.payload.token, user: action.payload.result };
        state.loading = false;
        state.error = null;
      })
      ...
      .addCase(login.fulfilled, (state, action: PayloadAction<{ token: string; result: User }>) => {
        state.data = { token: action.payload.token, user: action.payload.result };
        state.loading = false;
        state.error = null;
      })
      ...
      .addCase(updateuserprofile.fulfilled, (state, action: PayloadAction<{ token: string | null; result: User }>) => {
